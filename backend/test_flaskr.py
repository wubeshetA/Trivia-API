...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = (
            'postgresql://{}:{}@{}/{}'.format(
                self.database_user, self.database_passwd,
    ...
    def test_search_questions_without_result(self):
        res = self.client().post(
            '/questions', json={"searchTerm": "This is random search term"})
        data = json.loads(res.data)
    ...
    def test_404_quizzes(self):
        res = self.client().post('/quizzes',
                                 json={'wrong_previous_questions': [],
                                       'wrong_quiz_category': {'type': 'Engineering', 'id': 7}
                                       }
                                 )
        data = json.loads(res.data)
